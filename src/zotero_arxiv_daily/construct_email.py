...
def get_block_html(title:str, authors:str, rate:str, tldr:str, pdf_url:str, affiliations:str=None, link_label:str="PDF"):
    block_template = """
    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="font-family: Arial, sans-serif; border: 1px solid #ddd; border-radius: 8px; padding: 16px; background-color: #f9f9f9;">
    <tr>
        <td style="font-size: 20px; font-weight: bold; color: #333;">
...
"""
    return block_template.format(title=title, authors=authors,rate=rate, tldr=tldr, pdf_url=pdf_url, affiliations=affiliations, link_label=link_label)
...
def render_email(papers:list[Paper], card_page_url:str=None) -> str:
    ...
    for p in papers:
        ...
        # Non-arXiv sources have no direct PDF; fall back to the paper's
        # landing page instead of rendering a dead href="None" button.
        if p.pdf_url:
